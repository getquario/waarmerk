...
- `test/browser/` serves the shipped file under a strict CSP in Chromium. It is not a duplicate of the Node suites: every guarantee here rests on engine behaviour — WeakMap identity, property descriptors, intrinsics captured at load — and those are worth confirming in a real engine rather than assuming from Node. Unlike its siblings' harnesses it rewrites nothing, because this package has no dependency whose bare specifier a browser would fail to resolve; the browser runs exactly the bytes that ship.
